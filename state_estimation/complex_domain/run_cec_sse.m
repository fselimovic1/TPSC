function [ Vc, iter, converged, info ] = run_cec_sse(powsys, meas, sesettings, varargin)
...
wIdx = 1:2 * meas.num.pmu + meas.num.scada;
...
% ------------------------ Column indices ---------------------------------
jC11Iij0 = [     powsys.branch.i(-meas.pmu.loc(meas.pmu.Iji)); 
...
iDibrMO = [ (accI + (1:meas.num.sIjim)), (accI + (1:meas.num.sIjim)) ];
...
% --------------------------- Column indices ------------------------------
jDpbrO = [ powsys.branch.i(-meas.scada.loc(meas.scada.pji));...
...
jDqbrO = [ powsys.branch.i(-meas.scada.loc(meas.scada.qji));...
...
jDibrMO = [ powsys.branch.i(-meas.scada.loc(meas.scada.Ijim));...
...
if nopmu
    Zzi = sparse(2 * (powsys.num.zi + 1), 2 * (powsys.num.zi + 1));
    rJsl = sparse(1, powsys.num.islack, -1i / 2, 1, powsys.num.bus);
    J = [           [rJzi; rJsl],            sparse(powsys.num.zi + 1, powsys.num.bus);
        sparse(powsys.num.zi + 1, powsys.num.bus),    conj([rJzi; rJsl])  ];
else
    Zzi = sparse(2 * powsys.num.zi, 2 * powsys.num.zi);
    J = [           rJzi,            sparse(powsys.num.zi, powsys.num.bus);
        sparse(powsys.num.zi, powsys.num.bus),    conj(rJzi)  ];
end
% -------------------------------------------------------------------------
while iter < sesettings.maxNumberOfIter  
    ...
    % ----------------------- D update & construct  -----------------------
    vDpbrO = [  1/2 .* powsys.ybus.tofrom(-meas.scada.loc(meas.scada.pji)) ...
    ...
    vDqbrO = [  1i/2 .* powsys.ybus.tofrom(-meas.scada.loc(meas.scada.qji)) ...
    ...
    vDibrMO = [ 1/2 .* powsys.ybus.tofrom(-meas.scada.loc(meas.scada.Ijim)) ...
